...
void tremblez_potentiometres() {
  Serial.print("where_bend = "); Serial.print(where_bend);
  Serial.print("\t; an0 = "); Serial.println(an0);
  Serial.print("\nwhere_time = "); Serial.print(where_time);
...
//  effet: périssando            ->      mode = 2
void right_perissando_play(int t=1000) {
  int i; 
  if (tr1_correctly_played) {i = 0;}
  else if (tr2_correctly_played) {i = 1;}
  else if (tr3_correctly_played) {i = 2;}
  ...
  int the_note = actual_frequencies[i] + floor(sin(actual_peri_nb * one_peri_gap) * perissando_gaps_live[i]);
  right_tone.play(the_note, t);
  //Serial.println(actual_peri_nb);
};
...
void perissando_mode() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    for (int j = 0; j < diviseur; j++) {
      if (j_bis == 0) {k = 0;}
      else {k = floor(((perissando_sub_div * diviseur) / (j_bis * delta_temps)));}
      ...
      if (!left_all_ok()) {left_tone.stop();}
      
      if (right_all_ok()) {
        if (actual_peri_nb == total_peri_gaps - 1) {actual_peri_nb = 0;}
        else {actual_peri_nb += 1;}
        Serial.println(k);
        right_perissando_play(1000);
        if (j_bis == diviseur) {j_bis = 0;}
        else {j_bis += 1;}
      }
      else {
        right_tone.stop();
...
void pianos_checking() {
  ...
  while (tl_played || tr_played) {
    ...
    if (correct_left_playing() || correct_right_playing()) {
      ...
      else if (mode == 2) {perissando_mode();}  //Non-opérationnel !
      else {normal_mode();}
