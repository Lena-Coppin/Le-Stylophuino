...
void choosen_set(int i){
  ...
  write_matrix(HIGH, 48+current_set_number);  //juste un peu d'ASCII...
};


void actual_frequencies_update() {
  an0 = analogRead(where_bend);
  analogWrite(where_led_bend, constrain(map(an0, 0, 1023, 0, 255), 0, 255));
  if (mode == 1) {
    for (int i = 0; i < 20; i++) {
    ...
    for (int i = 0; i < 13; i++) {
      actual_frequencies[i] = the_set[i] + constrain(map(an0, 0, 1023, 0, pitchbend_variations_set[i]), 0, pitchbend_variations_set[i]);
    }
  }
};
...
void actual_poten_speed() {
  an1 = constrain(map(analogRead(where_time), 70, 950, 1000, 12), 12, 1000);
...
void change_metrovalle(char c) {
  ...
  if (mtv < 45) {mtv = 180;}
  else if (mtv > 180) {mtv = 45;}
  metrovalle = (unsigned int) 1000 * (60 / mtv);
  ...
  if (Serial.available() || Serial1.available()) {
    ...
    switch (charblu) {
      ...
      case 't':
        ...
        //Serial.println(tournement_total);
        break;
      case '0':
